...
class CustomPPOTrainer(PPOTrainer, Trainer):
    ...
    def ppo_train(self) -> None:
        ...
        for step in tqdm(range(max_steps), disable=not self.is_local_process_zero()):
            ...
            # Get rewards (label and rationale)
            if self.model_args.reward_model_rationale is not None:
                rewards = self.get_rewards(queries, responses, unwrapped_model, True)
            else:
    ...
    def get_rewards(
        ...
        # add reward_rationale model rewards
        if reward_rationale:
            replace_model(unwrapped_model, target="reward_rationale")
            batch_rationale = self.prepare_model_inputs(queries, responses)

            with torch.cuda.amp.autocast(dtype=self.model_args.compute_dtype): # support bf16
                _, _, values_rationale = self.model(**batch_rationale, output_hidden_states=True, return_dict=True)

            if values_rationale.size(0) != batch_rationale["input_ids"].size(0): # adapt to chatglm2
                values_rationale = torch.transpose(values_rationale, 0, 1)
            rewards_rationale = []
            for i in range(values_rationale.size(0)):
                end_index = batch_rationale["attention_mask"][i].nonzero()[-1] # use the score on the EOS token
                rewards_rationale.append(values_rationale[i, end_index].float().detach().cpu()) # use fp32 type
            rewards_res = []
            for r, r_r in zip(rewards, rewards_rationale):
                if r < self.model_args.label_reward_threshold:
                    rewards_res.append(r+r_r)
                else:
                    rewards_res.append(self.model_args.label_reward_threshold+r_r)
            # rewards = [0.5*r + 0.5*r_r for r, r_r in zip(rewards, rewards_rationale)]
